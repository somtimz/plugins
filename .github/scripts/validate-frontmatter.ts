...
function validateAgent(frontmatter: Record<string, unknown>): ValidationIssue[] {
...
function detectFileType(filePath: string): FileType | null {
  ...
  if (filePath.includes("/skills/") && basename(filePath) === "SKILL.md") return "skill";
...
async function main() {
  ...
  for (const { path: filePath, type } of files) {
    ...
    if (!result.error) {
      switch (type) {
        case "agent": issues.push(...validateAgent(result.frontmatter)); break;
        case "skill": issues.push(...validateSkill(result.frontmatter)); break;
        case "command": issues.push(...validateCommand(result.frontmatter)); break;
      }
    }
