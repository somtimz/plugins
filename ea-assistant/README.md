...
- **Motivation framework** — structured Business Drivers (DRV), Goals (G), Objectives (OBJ), Strategies (STR), Issues (ISS), and Problems (PRB) with ID-based traceability chains, each managed in its own **dedicated, scored Phase-A register** (`drivers/goals/objectives/strategy/issues/problems-register`) and regenerated from `engagement.json → direction` (the single source of truth); the **Architecture Vision is a strategic index** that summarises and links to these registers rather than embedding their tables; `/ea-status --direction --quality` scans all direction items for miscategorization, missing evidence, isolated items, and ambiguous phrasing; `/ea-grill --skill direction` runs an interactive direction quality review; inline quality challenge during engagement interviews flags and summarises issues before proceeding
- **Business Context analysis** — PESTEL, SWOT, competitor, regulatory, and stakeholder findings captured as **CTX-NNN** detail files and traced into the Motivation Framework (drivers, issues, opportunities, policies, constraints); makes external/internal context a first-class input to the Architecture Vision and Business Architecture
- **Business Model Canvas** — Phase B BMC template (9 building blocks) with 27-question interview bank, linkage table to Business Architecture elements, and **BMC-NNN** detail-file IDs for canvas elements and assumptions; sits between Strategy and Business Architecture/Operating Model
- **Four-concept business layer** — clear conceptual homes for **Business Context** (environment), **Business Model Canvas** (value model), **Business Architecture** (stable blueprint), and **Operating Model** (execution design); prevents Phase B content from drifting between artifacts
...
- **Business Architecture layer objects** — first-class `/ea-valuestreams` (VS-NNN), `/ea-processes` (PROC-NNN), and `/ea-usecases` (UC-NNN) registers with dedicated commands, skills, and cross-cutting registers; traceability chain extends `G-NNN / STR-NNN → CAP-NNN → VS-NNN → PROC-NNN → UC-NNN → REQ-NNN`; Phase B Business Architecture template renders summary-and-link sections to these registers
- **Operating Model** — first-class `/ea-operatingmodel [create|view|check|link|interview]` command and `templates/phase-b/operating-model.md` artifact for execution design (org design, roles/decision rights, controls, processes, workforce, sourcing, enablement, performance management); complements the Business Architecture stable blueprint; Phase B interview now offers the BA question bank followed by an optional Operating Model question bank; no new top-level `engagement.json` arrays or ID prefixes — reuses existing `CAP-NNN`, `VS-NNN`, `SVC-NNN`, `BR-NNN`, `PROC-NNN`, `UC-NNN`, `ROLE-NNN` objects
- **Stakeholder Goal Classification** — goals can be tagged by the four classic perspectives (Senior Management, Business Unit Manager, Staff, Ultimate Client) via `/ea-goals` and the Phase A interview; used for grouping in the Goals Register and stakeholder-tailored views
...
- **Artifact grill** — deep-review any artifact using grill-me skills (stress-test, premortem, decision, design, software-design, infra-design, artifact, diagram, boardroom-strategy, finance); auto-selects best skill by artifact type; apply findings back to the artifact one revision at a time with per-revision confirm/skip/edit. `--skill finance` runs a cost critique (coverage vs `FIN-NNN`, whole-life TCO, payback/value, funding) applying T4-TCO / T4-ECON — recommended for the Business Case
- **Artifact scoring** — `/ea-score` assigns two scores, **Completeness** and **Quality** (0–100 + band), **per section and overall**, using `grill-scoring-rubric.md` (grounded in the EA concept definitions, each section's guidance block, and the compliance tiers; Quality includes a readability dimension). Scores render into an author-only `<details>📊 Scorecard</details>` block in each artifact (per-section table + overall, stripped on export); `--all` scores the whole engagement and the overall pair is cached in `engagement.json` for tracking. `/ea-grill` emits the same two scores during a deep review. Command-generated artifacts (registers/matrices) are not scored — except the six Phase-A motivation registers (drivers/goals/objectives/strategy/issues/problems), which are seeded from guidance-rich templates and **are** scored
...
- **Business Rules Register** — `/ea-rules` manages declarative Business Rules (BR-NNN) with Subject, Condition, Directive, Outcome, Authority, and Enforcement; traces rules to Business Services (SVC-NNN), policies (POL-NNN), constraints (CST-NNN), and motivation elements; distinguishes rules from policies, constraints, requirements, and processes
- **Business Services Register** — `/ea-services` manages Business, Application, and Technology Services (SVC-NNN) using the Service Passport (Consumer–Outcome–Interface triad); traces services to Business Rules (BR-NNN), capabilities (CAP-NNN), ABB/SBB building blocks, and interfaces (IFC-NNN)
...
- **ARB Council** — `/ea-council` (alias `/ea-arb council`) convenes a multi-member review panel — Planner (feasibility), Security Analyst (vulnerabilities), Budget Analyst (cost/value), Architect (design quality), Innovator (latest tech), Conservative (tried-and-true) — that each examine a subject (artifact, phase, ADR, or whole engagement) through one mandate and cast a recorded vote; the consolidated verdict (tally, consensus, Innovator⇄Conservative points of contention, conditions) feeds the ARB Decisions table, with conditions becoming Actions and Security/Budget red-flags proposed as RIS-NNN risks. The 6-member roster is a data-driven, extensible reference (adding a member is a data edit)
...
- **Complete concept catalogue** — `skills/ea-artifact-templates/references/ea-concepts.md` (concept map) plus the per-family subfiles under `concept-families/` cover Service (SVC-NNN), Interface (IFC-NNN), Components, Capability Increment, Plateau, Deliverable, Architecture Partitioning, and Enterprise Continuum alongside the motivation and building-block concepts
...
- **TOGAF governance model** — two-layer governance reference (architecture governance vs programme governance) with phase-by-phase table; `skills/ea-artifact-templates/references/two-layers-of-intent.md` and `concept-families/motivation-across-adm.md` capture the full motivation-concepts ADM lifecycle (where each concept is first captured, refined, realized, and adapted across Preliminary through Phase H)
...
| `/ea-council [artifact\|phase X\|adr ADR-NNN\|--all] [--quick]` | Convene the ARB council — a 6-member panel that votes on a subject and produces a consolidated verdict |
...
| `/ea-arb [new\|council\|list\|view\|close]` | ARB meeting minutes — ARB-NNN, quorum, decisions, propagate to ADR register; `council` records a panel verdict |
...
| `/ea-valuestreams [list\|add\|update\|trace\|generate]` | Value Streams Register — manage VS-NNN value streams, trace to capabilities, goals, and processes |
| `/ea-processes [list\|add\|update\|trace\|generate]` | Business Processes Register — manage PROC-NNN processes, trace to value streams, capabilities, use cases, rules, and services |
| `/ea-usecases [list\|add\|update\|trace\|generate]` | Use Cases Register — manage UC-NNN use cases, trace to requirements, processes, capabilities, and value streams |
| `/ea-operatingmodel [create\|view\|check\|link\|interview]` | Operating Model — design how the organisation executes the Business Architecture (org design, roles/decision rights, controls, processes, workforce, sourcing, enablement, performance management) |
