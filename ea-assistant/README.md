...
- **Migration** — `/ea-migrate` aligns legacy engagements to the current plugin version; scan 3g flags high-priority items (Critical/High risks, High-priority requirements, Strategic decisions) lacking detail files (report-only — detail files are created on demand, never as bulk empty stubs); preview with `--report`
...
- **Register protocol** — the direction-register commands (`/ea-drivers`, `/ea-goals`, `/ea-issues`, `/ea-problems`, `/ea-gaps`) share one mode-mechanics reference (`register-protocol.md`); each command is a declarative spec plus its unique checks
...
- **Framework lenses** — pluggable overlay for external prescriptive frameworks (`ea-framework-lenses` skill); AWS Well-Architected lens included: `/ea-grill --skill waf` runs a pillar-by-pillar review, and Phase C/D/E interviews offer lens questions on cloud-scope engagements; Azure CAF / GCP are drop-in reference files
