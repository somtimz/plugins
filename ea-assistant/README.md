...
| `/ea-migrate [--report\|--reorganize]` | Align a legacy engagement to the current plugin version conventions; backfills template body sections/guidance the artifact predates (insertion-only, per-section confirm); `--reorganize` moves flat-path artifacts into correct phase subfolders |
