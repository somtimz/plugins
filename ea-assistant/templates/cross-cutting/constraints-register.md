...
The roll-up view of the register (regenerated by `/ea-constraints`, not hand-edited). Read it for health, not just counts: a high **Untraced** count means constraints aren't linked to the artifacts or SBBs they bound (T4-CON-TRACE); **Proposed** constraints awaiting ratification shouldn't yet be driving design; and every **Enterprise 🔒** constraint should carry an impact assessment. A constraint that bounds nothing is either mis-scoped or a principle in disguise.
