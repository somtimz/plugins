...
Objectives operationalise goals (DRV → G → **OBJ**): each is a specific, measurable, time-bound result with a unit of measure, a target value, and a deadline. Every objective must link to exactly one goal; Problems block objectives; Metrics (MET-NNN) track them. Objectives appear in Architecture Vision §6 — the register is the management interface; Architecture Vision is the primary display view.
...
| Display view | Architecture Vision `§6 Objectives` — columns `ID \| Objective \| Measure \| Target \| Deadline \| Linked Goal \| Details` ← `id, statement, measure, target, deadline, linkedGoal` |
...
- **Add success:** "OBJ-NNN added. Use '/ea-objectives trace OBJ-NNN' to verify the goal linkage, and add a tracking metric in Architecture Vision §11 Key Metrics."
