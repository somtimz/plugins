...
- top-level `finance`, `policies`, `rules`, `services`, `valueStreams`, `businessProcesses`, `useCases`, `adoptedRAs`, `localRA`, `constraintsRepoPath` and `direction.opportunities`/`direction.gaps` are present
...
```
GAP-M-{NNN}  [{Severity}]  {file} — {description}
             Suggested: {/ea-businesscontext|/ea-artifact create business-model-canvas|add CTX/BMC links}
```
