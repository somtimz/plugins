...
- **Seed sources:** rows — `engagement.json → direction.goals[]` and `direction.objectives[]` (rendered in the Goals/Objectives registers; surfaced in Architecture Vision §5/§10 summaries); columns — `artifacts/phase-b/business-architecture.md` §5 Business Services (SVC-NNN tokens), plus any application/technology service sections in the Phase C-App / D architectures
