...
| Architecture Vision, **Operating Model**, Business/Data/App/Tech Architecture, **Business Model Canvas**, Gap Analysis, Consolidated Gap Analysis, Architecture Roadmap, Statement of Architecture Work, Migration Plan, Business Case, Compliance Assessment, Requirements Register, Engagement Charter, Governance Framework, Implementation Governance Plan, Communications Plan, Architecture Definition Document, Transition Architectures | `## Appendix A5 — Related Architecture Decisions` section present | T3-ADR |
| **Operating Model** | §5 Business Processes Execution Model contains at least one summary-and-link reference to a `PROC-NNN` entry, and §2 Organisation Design / §3 Roles, Decision Rights & Accountability are non-placeholder | T3-OM-LINK |
| **Business Model Canvas** | Section 11 (Linkage to Business Architecture) contains at least one non-placeholder row mapping a BMC block to a Business Architecture element (capability, service, value stream, process, or requirement) | T3-BMC-LINK |
...
| T4-CTX-EVID | Business Context traceability: High-priority Drivers, Issues, Opportunities, Policies, and Constraints either cite a CTX-NNN source or are explicitly flagged as an assumption with owner and validation path | Motivation registers and governance registers | DRV-001 (High) — `Source: CTX-003 Market analysis Q2 2026` | DRV-001 (High) has no source, no cited CTX-NNN, and is not flagged as an assumption |
