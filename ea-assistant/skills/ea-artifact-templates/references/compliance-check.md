...
| T4-PREMAT | Premature decision detection: Technology, vendor, or pattern decisions documented in Phase A artifacts (before business capability and application boundary analysis) are flagged as premature | Architecture Vision, Statement of Architecture Work | Phase A contains only directional choices; any specific tech choice is in a PAD-NNN with constraint boundaries | Architecture Vision §15 contains "We will adopt microservices" without service boundary analysis |
