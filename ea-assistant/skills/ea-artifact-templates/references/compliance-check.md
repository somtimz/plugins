...
| T4-PREMAT | Premature decision detection: Technology, vendor, or pattern decisions documented in Phase A artifacts (before business capability and application boundary analysis) are flagged as premature | Architecture Vision, Statement of Architecture Work | Phase A contains only directional choices; any specific tech choice is in a PAD-NNN with constraint boundaries | Architecture Vision §8 contains "We will adopt microservices" without service boundary analysis |
...
| **L3** — Integrated | Required | Required | Required | Aspirational — T4-ECON, T4-EVID, T4-PREMAT encouraged | Manual + some automated |
| **L4** — Value-driven | Required | Required | Required | Required — T4-ECON, T4-EVID, T4-PREMAT, T4-POLIT expected; T4-PAD, T4-WPEVID encouraged | Automated + selective manual |
