...
- [X] T014 [US1] Implement `scripts/lib/embedder.py`: `embed_chunks(chunks, config) -> list[EmbeddedChunk]`; initialise `openai.OpenAI(base_url=config.api_base, api_key=os.environ[config.embedding_key_env])`; call `/v1/embeddings` per chunk (or batch); retry up to 5 times on `RateLimitError` using exponential backoff with jitter (cap total wait at 30 s); raise `EmbeddingError` after all retries exhausted
